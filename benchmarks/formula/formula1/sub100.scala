...
object benchmarks_formula_formula1_sub100 {
  ...
  def eval(formula: Formula): Boolean = {
    val _2 = {
      def eval2(expr) = {
        expr match {
          case NUM(n) => { n }
          case PLUS(expr1, expr2) => { eval2(expr1) + eval2(expr2) }
          case MINUS(expr1, expr2) => { eval2(expr1) - eval2(expr2) }
        }
      }
      formula match {
        case TRUE => { true }
        ...
        case IMPLY(formula1, formula2) => {
          not(eval(formula1)) || eval(formula2)
        }
        case LESS(expr1, expr2) => { eval2(expr1) < eval2(expr2) }
      }
    }
  } 
}