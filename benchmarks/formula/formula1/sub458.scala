...
object benchmarks_formula_formula1_sub458 {
    ...
    (f) =>
      {
        f match {
          case NUM(i) => { i }
          case PLUS(i1, i2) => { cal(i1) + cal(i2) }
          case MINUS(i1, i2) => { cal(i1) - cal(i2) }
        }
    }
  )
    ...
    (f) =>
      {
        f match {
          ...
          case LESS(e1, e2) => { if (cal(e1) - cal(e2) < 0) true else false }
        }
    }
  )
  /*
