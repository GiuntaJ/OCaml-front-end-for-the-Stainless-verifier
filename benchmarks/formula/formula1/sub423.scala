...
object benchmarks_formula_formula1_sub423 {
  ...
  def eval(x: Formula): Boolean = {
    x match {
      ...
      case IMPLY(a, b) => { if (eval(a) eq false) true else eval(b) }
      case LESS(p, q) => {
        val _2 = {
          def calExpr(e: Expr): Int63 = {
            e match {
              case NUM(n) => { n }
              case PLUS(f, g) => { calExpr(f) + calExpr(g) }
              case MINUS(f, g) => { calExpr(f) - calExpr(g) }
            }
          }
          calExpr(p) < calExpr(q)
        }
