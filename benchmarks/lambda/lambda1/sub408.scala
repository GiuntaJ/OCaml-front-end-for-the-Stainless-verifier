...
object benchmarks_lambda_lambda1_sub408 {
  ...
  def checkMetro(m: Metro): Boolean = {
    val _2 = {
      def envMetro(l: List[Name], m: Metro): Boolean = {
        m match {
          case STATION(n) => { if (l.contains(n) eq true) true else false }
          case AREA(n, u) => { envMetro(n :: l, u) }
          case CONNECT(u, v) => { envMetro(l, u) && envMetro(l, v) }
        }
      }
      envMetro(Nil(), m)
    }
