...
object benchmarks_crazy2add_sub221 {
  /* 2014-17189 이소희
  ...
  def crazy2add(((m: Crazy2, n: Crazy2))): Crazy2 = {
    m match {
      case NIL => { n }
      case ZERO(m1) => {
        n match {
          case NIL => { m }
          case ZERO(n1) => { ZERO(crazy2add(m1, n1)) }
          case ONE(n1) => { ONE(crazy2add(m1, n1)) }
          case MONE(n1) => { MONE(crazy2add(m1, n1)) }
        }
      }
      case ONE(m1) => {
        n match {
          case NIL => { m }
          case ZERO(n1) => { ONE(crazy2add(m1, n1)) }
          case ONE(n1) => { ZERO(crazy2add(crazy2add(ONE(NIL), m1), n1)) }
          case MONE(n1) => { ZERO(crazy2add(m1, n1)) }
        }
      }
      case MONE(m1) => {
        n match {
          case NIL => { m }
          case ZERO(n1) => { MONE(crazy2add(m1, n1)) }
          case ONE(n1) => { ZERO(crazy2add(m1, n1)) }
          case MONE(n1) => { ZERO(crazy2add(crazy2add(MONE(NIL), m1), n1)) }
        }
      }
    }
  }
