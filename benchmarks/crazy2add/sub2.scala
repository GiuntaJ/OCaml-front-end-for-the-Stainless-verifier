...
object benchmarks_crazy2add_sub2 {
  ...
  def check(result: Crazy2): Boolean = {
    result match {
      case ZERO(NIL) => { true }
      case ZERO(next) => { true && check(next) }
      case _ => { false }
    }
  }
  
  def zeroCheck(result: Crazy2): Crazy2 = {
    if (check(result)) ZERO(NIL) else result
  }
  ...
  def crazy2add(((number1: Crazy2, number2: Crazy2))) = {
    (number1, number2) match {
      ...
      case _ => { zeroCheck(crazy2add_in(number1, number2)) }
    }
  }
}