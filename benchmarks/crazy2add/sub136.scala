...
object benchmarks_crazy2add_sub136 {
  ...
  def crazy2add(((c1, c2))): Crazy2 = {
    (c1, c2) match {
      ...
      case (ONE(c1), ONE(c2)) => { ZERO(crazy2add(c1, crazy2add(c2, ONE(NIL))))
      }
      ...
      case (MONE(c1), MONE(c2)) => {
        ZERO(crazy2add(c1, crazy2add(c2, MONE(NIL))))
      }
    }
  }
}