...
object benchmarks_crazy2add_sub183 {
  ...
  def crazy2add(((c1: Crazy2, c2: Crazy2))): Crazy2 = {
    (c1, c2) match {
      ...
      case (ONE(cc1), MONE(cc2)) => { ZERO(crazy2add(cc1, cc2)) }
      case (MONE(cc1), ONE(cc2)) => { ZERO(crazy2add(cc1, cc2)) }
      case (ONE(cc1), ONE(cc2)) => {
        ZERO(crazy2add(ONE(NIL), crazy2add(cc1, cc2)))
      }
      case (MONE(cc1), MONE(cc2)) => {
